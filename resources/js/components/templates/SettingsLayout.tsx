...
export function SettingsLayout({
    ...
    return (
        <AppShell
            title="Pengaturan"
            coupleName={auth.user.couple?.name || "Settings"}
        ...
        >
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                <div className="md:hidden space-y-4">
                    <select
                        value={route(
                            navItems.find((i) => i.label === currentSection)
                    ...
                    >
                        {navItems.map((item) => (
                            <option key={item.route} value={route(item.route)}>
                                {item.label}
                            </option>
                        ))}
                    </select>
                    ...
                    <Link
                        ...
                        method="post"
                        as="button"
                        className="flex items-center justify-center w-full px-4 py-3 rounded-xl font-medium text-rose-600 bg-rose-50 dark:bg-rose-950/30 hover:bg-rose-100 transition-colors"
                    >
